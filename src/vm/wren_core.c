...
void wrenInitializeCore(WrenVM* vm)
{
  ...
  PRIMITIVE(vm->objectClass, "!", object_not, false);
  PRIMITIVE(vm->objectClass, "==(_)", object_eqeq, false);
  PRIMITIVE(vm->objectClass, "!=(_)", object_bangeq, false);
  PRIMITIVE(vm->objectClass, "is(_)", object_is, false);
  PRIMITIVE(vm->objectClass, "toString", object_toString, false);
  PRIMITIVE(vm->objectClass, "type", object_type, false);

  // Now we can define Class, which is a subclass of Object.
  vm->classClass = defineClass(vm, coreModule, "Class");
  wrenBindSuperclass(vm, vm->classClass, vm->objectClass);
  PRIMITIVE(vm->classClass, "name", class_name, false);
  PRIMITIVE(vm->classClass, "supertype", class_supertype, false);
  PRIMITIVE(vm->classClass, "toString", class_toString, false);
  ...
  PRIMITIVE(objectMetaclass, "same(_,_)", object_same, false);
  ...
  PRIMITIVE(vm->boolClass, "toString", bool_toString, false);
  PRIMITIVE(vm->boolClass, "!", bool_not, false);

  vm->fiberClass = AS_CLASS(wrenFindVariable(vm, coreModule, "Fiber"));
  PRIMITIVE(vm->fiberClass->obj.classObj, "new(_)", fiber_new, false);
  PRIMITIVE(vm->fiberClass->obj.classObj, "abort(_)", fiber_abort, false);
  PRIMITIVE(vm->fiberClass->obj.classObj, "current", fiber_current, false);
  PRIMITIVE(vm->fiberClass->obj.classObj, "suspend()", fiber_suspend, false);
  PRIMITIVE(vm->fiberClass->obj.classObj, "yield()", fiber_yield, false);
  PRIMITIVE(vm->fiberClass->obj.classObj, "yield(_)", fiber_yield1, false);
  PRIMITIVE(vm->fiberClass, "call()", fiber_call, false);
  PRIMITIVE(vm->fiberClass, "call(_)", fiber_call1, false);
  PRIMITIVE(vm->fiberClass, "error", fiber_error, false);
  PRIMITIVE(vm->fiberClass, "isDone", fiber_isDone, false);
  PRIMITIVE(vm->fiberClass, "transfer()", fiber_transfer, false);
  PRIMITIVE(vm->fiberClass, "transfer(_)", fiber_transfer1, false);
  PRIMITIVE(vm->fiberClass, "transferError(_)", fiber_transferError, false);
  PRIMITIVE(vm->fiberClass, "try()", fiber_try, false);

  vm->fnClass = AS_CLASS(wrenFindVariable(vm, coreModule, "Fn"));
  PRIMITIVE(vm->fnClass->obj.classObj, "new(_)", fn_new, false);

  PRIMITIVE(vm->fnClass, "arity", fn_arity, false);
  PRIMITIVE(vm->fnClass, "call()", fn_call0, false);
  PRIMITIVE(vm->fnClass, "call(_)", fn_call1, false);
  PRIMITIVE(vm->fnClass, "call(_,_)", fn_call2, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_)", fn_call3, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_,_)", fn_call4, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_,_,_)", fn_call5, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_,_,_,_)", fn_call6, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_,_,_,_,_)", fn_call7, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_,_,_,_,_,_)", fn_call8, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_,_,_,_,_,_,_)", fn_call9, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_,_,_,_,_,_,_,_)", fn_call10, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_,_,_,_,_,_,_,_,_)", fn_call11, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_,_,_,_,_,_,_,_,_,_)", fn_call12, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_,_,_,_,_,_,_,_,_,_,_)", fn_call13, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_,_,_,_,_,_,_,_,_,_,_,_)", fn_call14, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_,_,_,_,_,_,_,_,_,_,_,_,_)", fn_call15, false);
  PRIMITIVE(vm->fnClass, "call(_,_,_,_,_,_,_,_,_,_,_,_,_,_,_,_)", fn_call16, false);
  PRIMITIVE(vm->fnClass, "toString", fn_toString, false);

  vm->nullClass = AS_CLASS(wrenFindVariable(vm, coreModule, "Null"));
  PRIMITIVE(vm->nullClass, "!", null_not, false);
  PRIMITIVE(vm->nullClass, "toString", null_toString, false);

  vm->notImplementedClass = AS_CLASS(wrenFindVariable(vm, coreModule, "NotImplemented"));
  PRIMITIVE(vm->nullClass, "!", notImplemented_not, false);
  PRIMITIVE(vm->nullClass, "toString", notImplemented_toString, false);

  vm->numClass = AS_CLASS(wrenFindVariable(vm, coreModule, "Num"));
  PRIMITIVE(vm->numClass->obj.classObj, "fromString(_)", num_fromString, false);
  PRIMITIVE(vm->numClass->obj.classObj, "pi", num_pi, false);
  PRIMITIVE(vm->numClass->obj.classObj, "largest", num_largest, false);
  PRIMITIVE(vm->numClass->obj.classObj, "smallest", num_smallest, false);
  PRIMITIVE(vm->numClass, "-(_)", num_minus, false);
  PRIMITIVE(vm->numClass, "+(_)", num_plus, false);
  PRIMITIVE(vm->numClass, "*(_)", num_multiply, false);
  PRIMITIVE(vm->numClass, "/(_)", num_divide, false);
  PRIMITIVE(vm->numClass, "<(_)", num_lt, false);
  PRIMITIVE(vm->numClass, ">(_)", num_gt, false);
  PRIMITIVE(vm->numClass, "<=(_)", num_lte, false);
  PRIMITIVE(vm->numClass, ">=(_)", num_gte, false);
  PRIMITIVE(vm->numClass, "&(_)", num_bitwiseAnd, false);
  PRIMITIVE(vm->numClass, "|(_)", num_bitwiseOr, false);
  PRIMITIVE(vm->numClass, "^(_)", num_bitwiseXor, false);
  PRIMITIVE(vm->numClass, "<<(_)", num_bitwiseLeftShift, false);
  PRIMITIVE(vm->numClass, ">>(_)", num_bitwiseRightShift, false);
  PRIMITIVE(vm->numClass, "abs", num_abs, false);
  PRIMITIVE(vm->numClass, "acos", num_acos, false);
  PRIMITIVE(vm->numClass, "asin", num_asin, false);
  PRIMITIVE(vm->numClass, "atan", num_atan, false);
  PRIMITIVE(vm->numClass, "ceil", num_ceil, false);
  PRIMITIVE(vm->numClass, "cos", num_cos, false);
  PRIMITIVE(vm->numClass, "floor", num_floor, false);
  PRIMITIVE(vm->numClass, "-", num_negate, false);
  PRIMITIVE(vm->numClass, "round", num_round, false);
  PRIMITIVE(vm->numClass, "sin", num_sin, false);
  PRIMITIVE(vm->numClass, "sqrt", num_sqrt, false);
  PRIMITIVE(vm->numClass, "tan", num_tan, false);
  PRIMITIVE(vm->numClass, "log", num_log, false);
  PRIMITIVE(vm->numClass, "log2", num_log2, false);
  PRIMITIVE(vm->numClass, "exp", num_exp, false);
  PRIMITIVE(vm->numClass, "%(_)", num_mod, false);
  PRIMITIVE(vm->numClass, "~", num_bitwiseNot, false);
  PRIMITIVE(vm->numClass, "..(_)", num_dotDot, false);
  PRIMITIVE(vm->numClass, "...(_)", num_dotDotDot, false);
  PRIMITIVE(vm->numClass, "atan(_)", num_atan2, false);
  PRIMITIVE(vm->numClass, "pow(_)", num_pow, false);
  PRIMITIVE(vm->numClass, "fraction", num_fraction, false);
  PRIMITIVE(vm->numClass, "isInfinity", num_isInfinity, false);
  PRIMITIVE(vm->numClass, "isInteger", num_isInteger, false);
  PRIMITIVE(vm->numClass, "isNan", num_isNan, false);
  PRIMITIVE(vm->numClass, "sign", num_sign, false);
  PRIMITIVE(vm->numClass, "toString", num_toString, false);
  PRIMITIVE(vm->numClass, "truncate", num_truncate, false);

  // These are defined just so that 0 and -0 are equal, which is specified by
  // IEEE 754 even though they have different bit representations.
  PRIMITIVE(vm->numClass, "==(_)", num_eqeq, false);
  PRIMITIVE(vm->numClass, "!=(_)", num_bangeq, false);

  vm->stringClass = AS_CLASS(wrenFindVariable(vm, coreModule, "String"));
  PRIMITIVE(vm->stringClass->obj.classObj, "fromCodePoint(_)", string_fromCodePoint, false);
  PRIMITIVE(vm->stringClass->obj.classObj, "fromByte(_)", string_fromByte, false);
  PRIMITIVE(vm->stringClass, "+(_)", string_plus, false);
  PRIMITIVE(vm->stringClass, "[_]", string_subscript, false);
  PRIMITIVE(vm->stringClass, "byteAt_(_)", string_byteAt, false);
  PRIMITIVE(vm->stringClass, "byteCount_", string_byteCount, false);
  PRIMITIVE(vm->stringClass, "codePointAt_(_)", string_codePointAt, false);
  PRIMITIVE(vm->stringClass, "contains(_)", string_contains, false);
  PRIMITIVE(vm->stringClass, "endsWith(_)", string_endsWith, false);
  PRIMITIVE(vm->stringClass, "indexOf(_)", string_indexOf1, false);
  PRIMITIVE(vm->stringClass, "indexOf(_,_)", string_indexOf2, false);
  PRIMITIVE(vm->stringClass, "iterate(_)", string_iterate, false);
  PRIMITIVE(vm->stringClass, "iterateByte_(_)", string_iterateByte, false);
  PRIMITIVE(vm->stringClass, "iteratorValue(_)", string_iteratorValue, false);
  PRIMITIVE(vm->stringClass, "startsWith(_)", string_startsWith, false);
  PRIMITIVE(vm->stringClass, "toString", string_toString, false);

  vm->listClass = AS_CLASS(wrenFindVariable(vm, coreModule, "List"));
  PRIMITIVE(vm->listClass->obj.classObj, "filled(_,_)", list_filled, false);
  PRIMITIVE(vm->listClass->obj.classObj, "new()", list_new, false);
  PRIMITIVE(vm->listClass, "[_]", list_subscript, false);
  PRIMITIVE(vm->listClass, "[_]=(_)", list_subscriptSetter, false);
  PRIMITIVE(vm->listClass, "add(_)", list_add, false);
  PRIMITIVE(vm->listClass, "addCore_(_)", list_addCore, false);
  PRIMITIVE(vm->listClass, "clear()", list_clear, false);
  PRIMITIVE(vm->listClass, "count", list_count, false);
  PRIMITIVE(vm->listClass, "insert(_,_)", list_insert, false);
  PRIMITIVE(vm->listClass, "iterate(_)", list_iterate, false);
  PRIMITIVE(vm->listClass, "iteratorValue(_)", list_iteratorValue, false);
  PRIMITIVE(vm->listClass, "removeAt(_)", list_removeAt, false);

  vm->mapClass = AS_CLASS(wrenFindVariable(vm, coreModule, "Map"));
  PRIMITIVE(vm->mapClass->obj.classObj, "new()", map_new, false);
  PRIMITIVE(vm->mapClass, "[_]", map_subscript, false);
  PRIMITIVE(vm->mapClass, "[_]=(_)", map_subscriptSetter, false);
  PRIMITIVE(vm->mapClass, "addCore_(_,_)", map_addCore, false);
  PRIMITIVE(vm->mapClass, "clear()", map_clear, false);
  PRIMITIVE(vm->mapClass, "containsKey(_)", map_containsKey, false);
  PRIMITIVE(vm->mapClass, "count", map_count, false);
  PRIMITIVE(vm->mapClass, "remove(_)", map_remove, false);
  PRIMITIVE(vm->mapClass, "iterate(_)", map_iterate, false);
  PRIMITIVE(vm->mapClass, "keyIteratorValue_(_)", map_keyIteratorValue, false);
  PRIMITIVE(vm->mapClass, "valueIteratorValue_(_)", map_valueIteratorValue, false);

  vm->rangeClass = AS_CLASS(wrenFindVariable(vm, coreModule, "Range"));
  PRIMITIVE(vm->rangeClass, "from", range_from, false);
  PRIMITIVE(vm->rangeClass, "to", range_to, false);
  PRIMITIVE(vm->rangeClass, "min", range_min, false);
  PRIMITIVE(vm->rangeClass, "max", range_max, false);
  PRIMITIVE(vm->rangeClass, "isInclusive", range_isInclusive, false);
  PRIMITIVE(vm->rangeClass, "iterate(_)", range_iterate, false);
  PRIMITIVE(vm->rangeClass, "iteratorValue(_)", range_iteratorValue, false);
  PRIMITIVE(vm->rangeClass, "toString", range_toString, false);

  ObjClass* systemClass = AS_CLASS(wrenFindVariable(vm, coreModule, "System"));
  PRIMITIVE(systemClass->obj.classObj, "clock", system_clock, false);
  PRIMITIVE(systemClass->obj.classObj, "gc()", system_gc, false);
  PRIMITIVE(systemClass->obj.classObj, "writeString_(_)", system_writeString, false);
  PRIMITIVE(systemClass->obj.classObj, "notImplemented", system_notImplemented, false);
